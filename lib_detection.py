...
from utils import draw_losangle
...
def reconstruct(I, Iresized, Yr, lp_threshold):
    ...
    if len(final_labels):
        ...
        for _, label in enumerate(final_labels):
            ...
            points = label.pts * getWH(I.shape).reshape((2, 1))
            pts.append(points)
            # print(ptsh)
            # draw_losangle(I,label.pts * getWH(I.shape).reshape((2, 1)))
            H = find_T_matrix(ptsh, t_ptsh)
            # print(H.shape)
            # cv2.imshow('abc',I)
            Ilp = cv2.warpPerspective(I, H, out_size, borderValue=0)
            # cv2.imshow('cai gi do',Ilp)
            # cv2.waitKey()
            TLp.append(Ilp)
    # print('abacabac')
    # print(final_labels)
    return final_labels, TLp, lp_type,pts,confidence
...
def detect_lp(model, I, max_dim, lp_threshold):
    ...
    L, TLp, lp_type,points,confidence = reconstruct(I, Iresized, Yr, lp_threshold)

    return L, TLp, lp_type,points,confidence